...
import { Box, Button, Center, FormControl, FormErrorMessage, GridItem, HStack, PinInput, PinInputField, SimpleGrid, Spinner, Text, useToast } from '@chakra-ui/react';
...
export default function ConfirmPin({ password, setStep }:{ password:any, setStep:any}) {
                ...
                <HStack gap={['8px', '22px']}>
                  ...
                  >
                    {Array.from({ length: 6 }).map((_, index) => (
                      <PinInputField key={index} name={`confirmPin.${index}`} />
                    ))}
                  </PinInput>
                  </HStack>
                  <Text color="red.500"
                      ...
                      fontSize="sm"
                      onClick={()=>setStep(1)}
                      fontWeight="400">{!isValid ?"passwords must match click to return and createpin":""}</Text>
                  </FormControl>
                </GridItem>
