  ...
  const {
    ...
    const headers_For_Sell = [
     ...
     const bank = transaction.map((status: any) => {
      ...
      const displayWalletAddress = walletAddress.length > 5 
        ? `${walletAddress.slice(0, 5)}...` 
        : walletAddress;
    
      return {
        display: `${ontapBuy_sell === "Sell" ? status.bank.accountNumber : ontapBuy_sell === "Buy" ? displayWalletAddress : "No Wallet Address"} | ${ontapBuy_sell === "Sell" ? status.bank?.bankName : "address"}`,
        fullWalletAddress:ontapBuy_sell === "Sell" ?BankNo:ontapBuy_sell === "Buy" ?walletAddress:"",
